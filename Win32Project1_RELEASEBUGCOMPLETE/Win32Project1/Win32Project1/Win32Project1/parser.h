...
int input_valid_check(string get_str) /* 주소 입력 체크 */
{
	...
	char c = textbox_buffer[0];
	if (c < 0) { //한글이면 메모장에서 검사한다.
		FILE * fpk = fopen("korean_page.txt", "r");
		if (fpk == NULL) {
			MessageBox(NULL, "서버 데이터베이스 에러!", "에러!", MB_ICONINFORMATION);
			return -1;
		}
		...
		while (!feof(fpk))
		{
			pStr = fgets(strTemp, sizeof(strTemp), fpk);
			if (!strncmp(textbox_buffer, strTemp, strlen(textbox_buffer))) { //데이터베이스에 있으면
				printf("%s", strTemp);
				strtok(strTemp, "/");
	...
	if (!(textbox_buffer[0] == 'w') && !(textbox_buffer[0] == 'h')) //시작이 www 랑 http로 되지 않으면
	{
