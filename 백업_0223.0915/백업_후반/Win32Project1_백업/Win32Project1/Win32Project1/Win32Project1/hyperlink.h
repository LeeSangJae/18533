...
void image_hyperlink_maker(wchar_t * hypername) {
	...
	hwndStatic = CreateWindow(TEXT("static"), hypername, SS_NOTIFY | WS_CHILD | WS_VISIBLE, 10, 120, 200, 20, WinProc_sub, 0, g_hInst, 0); //이미지 파일 이름으로 하이퍼링크 설정
...
void hyperlink_maker(char * hypername) {
	...
	hwndStatic = CreateWindow(TEXT("static"), trans_hyper, SS_NOTIFY | WS_CHILD | WS_VISIBLE, 10, 120, 200, 20, WinProc_sub, 0, g_hInst, 0); //이미지 파일 이름으로 하이퍼링크 설정
	SendMessage(hwndStatic, WM_SETFONT, (WPARAM)GetStockObject(DEFAULT_GUI_FONT), 1);
