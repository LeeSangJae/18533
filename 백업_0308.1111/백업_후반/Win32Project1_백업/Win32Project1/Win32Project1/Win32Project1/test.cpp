	...
	case WM_COMMAND:
		...
		case ID_COMBOBOX:
			...
			case CBN_SELCHANGE:
				...
				if (!wcscmp(TEXT("방문기록보기"), textbox_buffer)) { //방문기록보기를 눌렀을 때
		...
		case VISIT_LOG_DELETE_BUTTON:
			...
			for (int i = 1; i < combo_page_count + 1; ++i) {
				SendMessage(hCombo, CB_DELETESTRING, i, FALSE);
				printf("페이지 %d 삭제 중...", i);
			}
			combo_page_count = 0;
			Delete_visit_page();
		...
		case NEW_WINDOW_BUTTON: //새 창 열기 버튼
			hwndChild = CreateWindow(lpszClass, lpszClass, WS_OVERLAPPEDWINDOW | WS_HSCROLL | WS_VSCROLL, CW_USEDEFAULT, CW_USEDEFAULT, 1200, 900, NULL, (HMENU)NULL, g_hInst, NULL);
